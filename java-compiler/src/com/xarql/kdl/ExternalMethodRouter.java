...
public class ExternalMethodRouter implements CommonText {
	...
	public static void writeMethods(final CompilationUnit unit, int line) throws Exception {
		...
		visitor = new Actor(unit.defineMethod(unit.addMethodDef(PRINTLN_MTD.withOwner(unit.getType()).withAccess(ACC_PUBLIC + ACC_STATIC))), unit);
		...
		PRINTLN_MTD.push(visitor);
		unit.getCurrentScope().end(1, visitor, ReturnValue.VOID);

		// add print method to class
		visitor = new Actor(unit.defineMethod(unit.addMethodDef(PRINT_MTD.withOwner(unit.getType()).withAccess(ACC_PUBLIC + ACC_STATIC))), unit);
		unit.getCurrentScope().newVariable(arg, new InternalName(String.class));
		visitor.visitFieldInsn(GETSTATIC, new InternalName(System.class).nameString(), "out", new InternalName(PrintStream.class).objectString());
		visitor.visitVarInsn(ALOAD, unit.getLocalVariable(arg).localIndex); // load input
		PRINT_MTD.push(visitor);
		unit.getCurrentScope().end(1, visitor, ReturnValue.VOID);

		// add error method to class
		visitor = new Actor(unit.defineMethod(unit.addMethodDef(ERROR_MTD.withOwner(unit.getType()).withAccess(ACC_PUBLIC + ACC_STATIC))), unit);
		unit.getCurrentScope().newVariable(arg, new InternalName(String.class));
		...
		visitor = new Actor(unit.defineMethod(unit.addMethodDef(PARSE_INT_MTD.withOwner(unit.getType()).withAccess(ACC_PUBLIC + ACC_STATIC))), unit);
		unit.getCurrentScope().newVariable(arg, new InternalName(String.class));
		visitor.visitVarInsn(ALOAD, unit.getLocalVariable(arg).localIndex); // load input
		PARSE_INT_MTD.push(visitor);
